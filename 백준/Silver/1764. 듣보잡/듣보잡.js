...
const answer = () => {
  ...
  const [NList, MList] = [input.slice(0, N), input.slice(N)];

  const map = NList.reduce((acc, cur) => {
    acc.set(cur, 1);
    return acc;
  }, new Map());

  const list = MList.filter((cur) => map.has(cur));
  return [list.length, ...list.sort((a, b) => a.localeCompare(b))].join('\n');
};
